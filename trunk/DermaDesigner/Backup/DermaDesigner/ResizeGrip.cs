...
namespace DermaDesigner {
	class ResizeGrip {
		...
		public static bool Resize_MouseDown(object sender, MouseEventArgs e) {
			if (host != null && Derma.GetSelected() == host && !host.isLocked && Derma.IsMouseOverArea(host.x + host.width - twoThirdsSize, host.y + host.height - twoThirdsSize, 16, 16)) {
				resizing = true;
				mouseOrigin.X = e.X;
				mouseOrigin.Y = e.Y;
				return true;
		...
		public static void Resize_MouseMove(object sender, MouseEventArgs e) {
			if (resizing) {
				...
				if (host.PreResize(host.width + newsize.X, host.height + newsize.Y) && host.width + newsize.X > 10 && host.height + newsize.Y > 10) {
					...
					mouseOrigin.X = e.X;
					mouseOrigin.Y = e.Y;
					if (host.WidthBox != null) host.WidthBox.Text = host.width.ToString();
					if (host.HeightBox != null)	host.HeightBox.Text = host.height.ToString();
					Derma.Repaint();
					host.PostResize();
				}
			}
